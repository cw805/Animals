...
(function($) {
    ...
    $.widget("ui.weekCalendar", {
        ...
        _refreshEventDetails : function(calEvent, $calEvent) {
            var self = this;
            var options = this.options;
            //$calEvent.find(".time").text(calEvent.title + ': ' + self._formatDate(calEvent.start, options.timeFormat) + options.timeSeparator + self._formatDate(calEvent.end, options.timeFormat));
            // $calEvent.find(".title").text(calEvent.title);
		$calEvent.find(".time").text(calEvent.title);
		$calEvent.find(".title").text(self._formatDate(calEvent.start, options.timeFormat) + options.timeSeparator + self._formatDate(calEvent.end, options.timeFormat));
		if (calEvent.title.search(/Chapel|Community Time/) != -1) {
			$calEvent.find(".time").css('background-color', '#FEFEC2');
			$calEvent.find(".title").css('background-color', '#FEFEC2');
			$calEvent.css('background-color', '#FEFEC2');
		}
